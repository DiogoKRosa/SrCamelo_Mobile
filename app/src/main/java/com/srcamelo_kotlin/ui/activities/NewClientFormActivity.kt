...
class NewClientFormActivity : ComponentActivity(){
    override fun onCreate(savedInstanceState : Bundle?){
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContent {
            SrCamelo_KotlinTheme {
                NewFormClientScreen(goBack = {finish()})
            }
        }
    }
}
...
fun NewFormClientScreen(
    ...
    Scaffold (topBar = {BackTopAppBar(finish = goBack)},
        content = {
            ...
            Column(
                ...
                ButtonWhite(title="Cadastrar-se", onClick = callRegister)
                SpecialText(text = "Já possuo uma conta", onClick = callLogin)
            }
        })
}